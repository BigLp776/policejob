...
function TakeOutImpound(vehicle)
    ...
    if coords then
        QBCore.Functions.TriggerCallback('QBCore:Server:SpawnVehicle', function(netId)
            ...
            QBCore.Functions.TriggerCallback('treap-garage:server:GetVehicleProperties', function(properties)
...
function MenuGarage(currentSelection)
    ...
    vehicleMenu[#vehicleMenu+1] = {
        ...
        txt = "",
        params = {
            event = "treap-menu:client:closeMenu"
        }

    }
    exports['treap-menu']:openMenu(vehicleMenu)
end
...
function MenuImpound(currentSelection)
    ...
    QBCore.Functions.TriggerCallback("police:GetImpoundedVehicles", function(result)
        ...
        if shouldContinue then
            ...
            exports['treap-menu']:openMenu(impoundMenu)
        end
    end)
...
RegisterNetEvent('police:client:EvidenceStashDrawer', function(data)
    ...
    if #(pos - takeLoc) <= 1.0 then
        local drawer = exports['treap-input']:ShowInput({
...
RegisterNetEvent('treap-policejob:ToggleDuty', function()
    TriggerServerEvent("QBCore:ToggleDuty")
    TriggerServerEvent("police:server:UpdateCurrentCops")
    TriggerServerEvent("police:server:UpdateBlips")
end)

RegisterNetEvent('treap-police:client:scanFingerPrint', function()
    local player, distance = GetClosestPlayer()
...
RegisterNetEvent('treap-police:client:openArmoury', function()
    local PlayerData = QBCore.Functions.GetPlayerData()
    local fullname = PlayerData.charinfo.firstname.." "..PlayerData.charinfo.lastname
    local cid = PlayerData.citizenid
    ...
    TriggerServerEvent('treap-log:server:CreateLog', 'armoury', 'Armoury Shop (Police)', 'blue', ('**Officer Name:** %s | **Citizen ID:** %s\n **Department:** %s | **Grade:** %s\n **Info:** Officer Accessed Armoury'):format(fullname, cid, PlayerData.job.label, PlayerData.job.grade.level))
end)

RegisterNetEvent('treap-police:client:spawnHelicopter', function(k)
...
RegisterNetEvent('treap-police:client:openInventory', function()
    ...
    CreateThread(function()
        while true do
            ...
            if inFingerprint and PlayerJob.type == "leo" then
                if PlayerJob.onduty then sleep = 5 end
                if IsControlJustReleased(0, 38) then
                    TriggerEvent("treap-police:client:scanFingerPrint")
                    break
                end
            else
                break
    ...
    CreateThread(function()
        while true do
            ...
            if inAmoury and PlayerJob.type == "leo" then
                if PlayerJob.onduty then sleep = 5 end
                if IsControlJustReleased(0, 38) then
                    TriggerEvent("treap-police:client:openArmoury")
                    break
                end
            else
                break
    ...
    CreateThread(function()
        while true do
            ...
            if inHelicopter and PlayerJob.type == "leo" then
                if PlayerJob.onduty then sleep = 5 end
                if IsControlJustReleased(0, 38) then
                    TriggerEvent("treap-police:client:spawnHelicopter")
                    break
                end
            else
                break
...
if Config.UseTarget then
    CreateThread(function()
        ...
        for k, v in pairs(Config.Locations["duty"]) do
            exports['treap-target']:AddBoxZone("PoliceDuty_"..k, vector3(v.x, v.y, v.z), 1, 1, {
                name = "PoliceDuty_"..k,
                heading = 11,
                debugPoly = false,
                minZ = v.z - 1,
                ...
                options = {
                    {
                        type = "client",
                        event = "treap-policejob:ToggleDuty",
        ...
        for k, v in pairs(Config.Locations["stash"]) do
            exports['treap-target']:AddBoxZone("PoliceStash_"..k, vector3(v.x, v.y, v.z), 1.5, 1.5, {
                name = "PoliceStash_"..k,
                heading = 11,
                debugPoly = false,
                minZ = v.z - 1,
                ...
                options = {
                    {
                        type = "client",
                        event = "treap-police:client:openStash",
                        icon = "fas fa-dungeon",
        ...
        for k, v in pairs(Config.Locations["trash"]) do
            exports['treap-target']:AddBoxZone("PoliceTrash_"..k, vector3(v.x, v.y, v.z), 1, 1.75, {
                name = "PoliceTrash_"..k,
                heading = 11,
                debugPoly = false,
                minZ = v.z - 1,
                ...
                options = {
                    {
                        type = "client",
                        event = "treap-police:client:openTrash",
                        icon = "fas fa-trash",
                        label = "Open Trash",
                        canInteract = function()
                            if (PlayerJob.name == "police" or PlayerJob.type == "leo") then return true end
        ...
        for k, v in pairs(Config.Locations["fingerprint"]) do
            exports['treap-target']:AddBoxZone("PoliceFingerprint_"..k, vector3(v.x, v.y, v.z), 2, 1, {
                ...
                options = {
                    {
                        type = "client",
                        event = "treap-police:client:scanFingerPrint",
        ...
        for k, v in pairs(Config.Locations["armory"]) do
            exports['treap-target']:AddBoxZone("PoliceArmory_"..k, vector3(v.x, v.y, v.z), 5, 1, {
                ...
                options = {
                    {
                        type = "client",
                        event = "treap-police:client:openArmoury",
                        icon = "fas fa-swords",
                        label = "Open Armory",
                        canInteract = function()
                            if (PlayerJob.name == "police" or PlayerJob.type == "leo") then return true end
